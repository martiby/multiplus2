...
class VEBus:
    ...
    def get_version(self):
        ...
        try:
            self.send_frame('V', [])
            rx = self.receive_frame(cmd='V', timeout=0.5)
    ...
    def get_led(self):
        ...
        try:
            self.send_frame('L', [])
            rx = self.receive_frame(cmd='L', timeout=0.5)
    ...
    def send_snapshot_request(self):
        """
        Trigger a snaphot for values. Could be combined with a other request. NO RESPONSE !
    ...
    def read_snapshot(self):
        ...
        try:
            ...
            frame = self.receive_frame(cmd='X')
            if frame[3] != 0x99:
    ...
    def set_power(self, power):
        ...
        try:
            ...
            self.log.error("set_ess_power: {}".format(e))
            return False
    ...
    def receive_frame(self, timeout=0.5, cmd=None, info=None):
        ...
        while time.perf_counter() < tout:
            ...
            time.sleep(0.010)
            if len(rx) > 2 and len(rx) == rx[0] + 2 and (sum(rx) & 0xFF == 0):
                if cmd is not None and rx[1] != 0xFF and rx[2] != ord(cmd):
                    raise Exception("invalid command response {}".format(self.format_hex(rx)))
                if info is not None and rx[1] != info:
                    raise Exception("invalid info response {}".format(self.format_hex(rx)))
                else:
                    self.log.debug("RX: {}".format(self.format_hex(rx)))
                    return rx
        if rx:
